    ...
    for (Vec2 v = p2; v != p1; v = path.getNextMove(v)) {
      if (!builder->getSquare(v)->canNavigate({MovementTrait::WALK})) {
        ...
        if (isWall(oldType) && oldType != SquareId::BLACK_WALL)
          newType = chooseRandom<SquareType>({SquareId::FLOOR, doorType}, {1 - doorProb, doorProb});
        else
...
class DungeonFeatures : public LevelMaker {
  ...
  virtual void make(LevelBuilder* builder, Rectangle area) override {
    ...
    for (int i : Range(available.size() * density)) {
      Vec2 pos = chooseRandom(available);
      builder->putSquare(pos, squares.getRandom());
      if (attr)
  ...
  virtual void make(LevelBuilder* builder, Rectangle area) override {
    ...
    for (Vec2 v : area)
      if (onPredicate.apply(builder, v))
        pos.push_back(v);
    checkGen(pos.size() > 0);
    builder->putSquare(pos[Random.get(pos.size())], {SquareId::STAIRS, StairInfo{key, stairLook, direction}},
        setAttr);
  }
...
static MakerQueue* mineTownMaker(SettlementInfo info) {
  return genericMineTownMaker(info, 10, 12, Random.get(5, 7), 6, 8);
}

static MakerQueue* smallMineTownMaker(SettlementInfo info) {
  return genericMineTownMaker(info, 2, 7, Random.get(3, 5), 5, 7);
}
...
static void addResources(RandomLocations* locations, int count, int countHere, int minSize, int maxSize,
    int maxDist, int maxDist2, SquareType type, LevelMaker* hereMaker) {
