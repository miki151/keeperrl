...
SERIALIZE_DEF(Immigration, immigrants, available, minionAttraction, idCnt, collective, generated, candidateTimeout)
...
vector<string> Immigration::getMissingRequirements(const Available& available) const {
  vector<string> ret = getMissingRequirements(Group {available.immigrantIndex, (int)available.creatures.size()});
...
vector<string> Immigration::getMissingRequirements(const Group& group) const {
  ...
  immigrants[group.immigrantIndex].visitRequirements(visitor);
  return ret;
}
...
map<int, std::reference_wrapper<const Immigration::Available>> Immigration::getAvailable() const {
  ...
  for (auto& elem : available)
    if (!elem.second.isUnavailable() &&
        preliminaryRequirementsMet({elem.second.immigrantIndex, (int)elem.second.creatures.size()}))
...
vector<Position> Immigration::Available::getSpawnPositions() const {
  ...
  return pickSpawnPositions(extractRefs(creatures), positions);
}
...
void Immigration::accept(int id) {
  ...
  if (!getMissingRequirements(candidate).empty())
    return;
  const int groupSize = candidate.creatures.size();
  auto& creatures = candidate.creatures;
  vector<Position> spawnPos = candidate.getSpawnPositions();
  if (spawnPos.size() < candidate.creatures.size())
    return;
  ...
  for (int i : All(candidate.creatures)) {
    Creature* c = candidate.creatures[i].get();
    if (i == 0 && groupSize > 1) // group leader
      c->getAttributes().increaseBaseExpLevel(2);
    collective->addCreature(std::move(candidate.creatures[i]), spawnPos[i], immigrantInfo.getTraits());
    occupyRequirements(c, candidate.immigrantIndex);
    generated[candidate.immigrantIndex].push_back(c);
  }
  candidate.creatures.clear();
...
void Immigration::Available::regenerate() {
  int numGenerated = immigration->generated[immigrantIndex].size();
  for (int i : Range(Random.get(getInfo().getGroupSize())))
    creatures.push_back(CreatureFactory::fromId(getInfo().getId(numGenerated), immigration->collective->getTribeId(),
        MonsterAIFactory::collective(immigration->collective)));
}
