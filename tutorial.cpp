...
string Tutorial::getMessage() const {
  switch (state) {
    case State::CUT_TREES:
      return "Your first task is to gather some wood! Select the \"Dig or cut tree\" order and click on some trees to "
          "order your imps to cut them.";
    case State::BUILD_STORAGE:
...
void Tutorial::refreshInfo(const Game* game, optional<TutorialInfo>& info) const {
  info = TutorialInfo {getMessage(), canContinue(game), (int) state > 0};
}
