...
void EventListener::addWorshipCreatureEvent(Creature* c, const Creature* to, WorshipType type) {
  for (EventListener* l : listeners)
    if (l->getListenerLevel() == c->getLevel() || l->getListenerLevel() == nullptr)
      l->onWorshipCreatureEvent(c, to, type);
}

void EventListener::addWorshipEvent(Creature* c, const Deity* to, WorshipType type) {
  for (EventListener* l : listeners)
    if (l->getListenerLevel() == c->getLevel() || l->getListenerLevel() == nullptr)
